...
public class DeviceInfoInstrument extends Instrumentation implements
	...
	public void onStart() {
		...
		addResult(RESOLUTION, String.format("%sx%s", metrics.widthPixels,
				metrics.heightPixels));
		addResult(SCREEN_DENSITY, metrics.density);
		int sbHeight = getStatusBarHeight(getContext());
		addResult(STATUSBAR_HEIGHT, sbHeight * metrics.density);
	...
	/**
	 * Return a summary of the device's feature as a semi-colon-delimited list
	 * of colon separated name and availability pairs like
	 * "feature1:sdk:true;feature2:sdk:false;feature3:other:true;".
	 */
	@SuppressLint("NewApi")
	private String getFeatures() {
		...
		try {
			...
			for (String featureName : getPackageManagerFeatures()) {
				...
				boolean hasFeature = packageManager
			...
			if (featureInfos != null) {
				for (FeatureInfo featureInfo : featureInfos) {
					if (featureInfo.name != null
							&& !checkedFeatures.contains(featureInfo.name)) {
						addFeature(features, featureInfo.name, "other", true);
					}
				}
			}
		} catch (Exception exception) {
			Log.e(TAG, "Error getting features: " + exception.getMessage(),
					exception);
		}
	...
	private static void addFeature(StringBuilder features, String name,
			String type, boolean available) {
		features.append(name).append(':').append(type).append(':')
				.append(available).append(';');
	}

	/**
	 * Use reflection to get the features defined by the SDK. If there are
	 * features that do not fit the convention of starting with "FEATURE_" then
	 * they will still be shown under the "Other Features" section.
	 *
	...
	/**
	 * Return a semi-colon-delimited list of the root processes that were
	 * running on the phone or an error message.
	 */
	private static String getProcesses() {
		...
		try {
			...
			Log.e(TAG, "Error getting processes: " + exception.getMessage(),
					exception);
	...
	/**
	 * @return a string containing the Open GL ES version number or an error
	 *         message
	 */
	@SuppressLint("NewApi")
	...
	private String getStorageDevices() {
		...
		count = Math.max(
				count,
				getContext().getExternalFilesDirs(
						Environment.DIRECTORY_PICTURES).length);
	...
	private String getMultiUserInfo() {
		try {
			final Method method = UserManager.class
					.getMethod("getMaxSupportedUsers");
